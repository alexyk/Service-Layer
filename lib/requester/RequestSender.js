                    ...
                    if (!response.ok) {
                      resolve({
                        ...
                        errors: response.text().then(function (responseAsRawText) {
                          var jsonData; // strip of HTML tags and CSS

                          var regex = /(<([^>]+)>|\{[^\}]+\})/gim;
                          var responseAsPlainText = responseAsRawText.replace(regex, ""); // strip of new lines

                          responseAsPlainText = responseAsPlainText.replace(/\r?\n|\r/gm, ""); // strip of tabs

                          responseAsPlainText = responseAsPlainText.replace(/\t/g, " "); // strip of more than 2 spaces

                          responseAsPlainText = responseAsPlainText.replace(/ {2,}/g, " "); // try parsing JSON
